use crate::{
    ...
    constants::{NO_CHARACTER_PHRASES, NO_HISTORY_PHRASES, sample},
...
pub async fn pin(ctx: Context<'_>, message: Message) -> Result<(), Report> {
    ...
    let Some(history): Option<History> = ctx.data().db.history(&message).await? else {
    ...
    let Some(character) = ctx.data().db.character(history.character()).await? else {
    ...
    let reply = history
        .into_bare_response(&character, message.link().to_string(), &ctx.data().db)
        .await;

    let channel_id = ctx.data().config.pins_channel_id;
    let pin = channel_id
        .widen()
